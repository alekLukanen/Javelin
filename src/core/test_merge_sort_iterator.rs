...
#[test]
fn test_simple_case_decreasing_puts_memtables_only() -> Result<(), Box<dyn Error>> {
    let config = DBConfigBuilder::new().logging_enabled(true).build();
    let tc = TestContext::new_from_config(config);

    let active_memtable = SampleMemtableBuilder::DecreasingPuts {
        size: 5,
        starting_value: 5,
        starting_log_sequence_num: 100,
    }
    ...
    let immuitable_memtable = Arc::new(ImmutableMemtable::new(
        0,
        tc.db_context.clone(),
        SampleMemtableBuilder::DecreasingPuts {
            size: 10,
            starting_value: 0,
            starting_log_sequence_num: 0,
        }
